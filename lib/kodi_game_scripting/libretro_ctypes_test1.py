...
class RetroValues(ctypes.Structure):
...
class LibretroWrapper:
    ...
    class RetroOptions(ctypes.Structure):
        ...
        _fields_ = [
            ...
            ('values', ctypes.POINTER(RetroValues))
        ]
    ...
    def __init__(self, library_path):
        ...
        def retro_environment_cb(cb_type, arg, outer=self):
            ...
            if cb_type == 18:  # RETRO_ENVIRONMENT_SET_SUPPORT_NO_GAME
                ...
                index = 0
                while True:
                    var = ctypes.cast(arg, ctypes.POINTER(self.RetroOptions))[index]
                    index += 1
                    print(var.key)
                    print(var.desc)
                    print(var.info)
                    print(var.values)
                    test =input('test')
                    if var.key is None and var.desc is None and var.values is None:
                        break
                    # outer.variables.append(outer.parse_libretro_options(var))
    ...
    def parse_libretro_options(cls, variable):
        """ Parse variable into Option(id, desc, info, values, default) """
        key = xstr(variable.key)
        if ';' in xstr(variable.value):
            description, values = xstr(variable.value).split(';', 1)
        else:
            description = xstr(variable.desc)+' ('+xstr(variable.info)+')'
            values = xstr(variable.value)
        values = values.strip().split('|')
        default = values[0]
